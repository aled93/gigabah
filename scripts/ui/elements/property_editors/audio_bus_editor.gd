...
@onready var mutedCheckButton: CheckButton = get_node("%Muted")
var _settings: AudioSettingsModel;

func _ready() -> void:
	slider.value_changed.connect(_slider_changed)
	mutedCheckButton.pressed.connect(_muted_changed)
...
func _update_representation() -> void:
	...
	mutedCheckButton.button_pressed = !_settings.muted
...
func _muted_changed() -> void:
	_settings.muted = !mutedCheckButton.button_pressed
